...
def add_pump(dt, m):
    gazole_min = dt['Gazole_prix'].min()
    sp98_min = dt['SP98_prix'].min()
    e10_min = dt['E10_prix'].min()
    ...
    for i in dt.index:
        ...
        if float(gazole) == float(gazole_min):
            marker = folium.Marker(
                location=[lat, long],
                popup=popup,
                icon=folium.Icon(color="yellow", icon="car", prefix="fa"),

            )
        elif float(sp98) == float(sp98_min):
            marker = folium.Marker(
                location=[lat, long],
                popup=popup,
...
def create_map():
    ...
    values = requests.get(
        f"https://data.economie.gouv.fr/api/records/1.0/search/?dataset=prix-des-carburants-en-france-flux-instantane-v2&q=&rows=10000&geofilter.distance={loc_data['latitude']}%2C+{loc_data['longitude']}%2C+3500000").json()
